	...
	void EnemyMovement() {
		...
		if ((myWaypoints.Length != 0) && (_moving)) {
			...
			if (Mathf.Abs(_vx) <= 0.05f) {
				...
				//Debug.Log ("y1=" + _transform.localScale.y.ToString());
				//Debug.Log ("y2=" + _rigidbody.velocity.y.ToString());
				// Set the enemy's velocity to moveSpeed in the x direction.
				_rigidbody.velocity = new Vector2(_transform.localScale.y * moveSpeed, _rigidbody.velocity.y);
			}
